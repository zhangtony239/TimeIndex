...
│   └── doctor.py           # 环境自检与依赖检查 (类似 openclaw doctor)
└── README.md


## 3. 核心模块与组件设计 (Core Modules)

### 3.1 Daemon 层 (daemon.py)
...
### 3.2 存储与 RAG 层 (db/vector_store.py)
...
/ti config: 读取配置项并执行自检（调用 utils/doctor.py，实现类似 openclaw doctor 的效果）。
...
Doctor 自检: utils/doctor.py 必须能够检查：WMI 权限、Ollama 服务连通性、模型(gemma4)是否已拉取、LanceDB 目录读写权限。